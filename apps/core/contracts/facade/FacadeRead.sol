...
contract FacadeRead is IFacadeRead {
    ...
    function redeem(
        ...
        uint256 amount,
        uint48 basketNonce
    ) external returns (address[] memory tokens, uint256[] memory withdrawals, bool isProrata) {
        IMain main = rToken.main();
        main.poke();
        IRToken rTok = rToken;
        IBasketHandler bh = main.basketHandler();
    ...
    function backupConfig(
        RTokenP1 rToken,
        bytes32 targetName
    ) external view returns (IERC20[] memory erc20s, uint256 max) {
    ...
    function backingOverview(
        IRToken rToken
    ) external view returns (uint192 backing, uint192 overCollateralization) {
    ...
    ) private view returns (BasketRange memory range) {
        IMain main = rToken.main();
        IBasketHandler bh = main.basketHandler();
